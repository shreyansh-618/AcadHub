...
export default function ResourceDetailPage() {
      ...
      try {
        ...
        if (response.ok && data.data?.resource) {
          ...
          if (data.data.resource.type === 'txt') {
            try {
              const textResponse = await fetch(`${API_BASE_URL}/api/v1/resources/${id}/view`);
  ...
  const handleLike = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/v1/resources/${id}/like`, {
        method: 'POST',
        headers: {
  ...
  const canViewInline = () => {
    // Show preview for files that browsers can display natively
    return resource && (resource.type === 'pdf' || resource.type === 'image' || resource.type === 'txt');
  };
      ...
      <div className="container-max max-w-4xl mx-auto">
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                {resource.description && (
                  <p className="text-blue-100 text-lg">{resource.description}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-semibold text-slate-400 block mb-1">Subject</label>
                  <p className="text-white text-lg">{resource.subject}</p>
                </div>
                <div>
                  <label className="text-sm font-semibold text-slate-400 block mb-1">Department</label>
                  <p className="text-white text-lg">{resource.department}</p>
                </div>
                <div>
                  <label className="text-sm font-semibold text-slate-400 block mb-1">Semester</label>
                  <p className="text-white text-lg">Semester {resource.semester}</p>
                </div>
              </div>
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-semibold text-slate-400 block mb-1">Academic Year</label>
                  <p className="text-white text-lg">{resource.academicYear}</p>
                </div>
                <div>
                  <label className="text-sm font-semibold text-slate-400 block mb-1">File Size</label>
                  <p className="text-white text-lg">{formatBytes(resource.fileSize)}</p>
                </div>
                <div>
                  <label className="text-sm font-semibold text-slate-400 block mb-1">Uploaded</label>
                  <p className="text-white text-lg">{formatDate(resource.createdAt)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-700 rounded-lg p-4 mb-8">
              <label className="text-sm font-semibold text-slate-400 block mb-2">Uploaded By</label>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-400 to-purple-600 flex items-center justify-center text-white font-bold">
                  {resource.uploadedByName?.charAt(0).toUpperCase() || 'U'}
                </div>
                <div>
                  <p className="text-white font-semibold">{resource.uploadedByName}</p>
                  {resource.uploadedBy?.email && (
                    <p className="text-slate-400 text-sm">{resource.uploadedBy.email}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-6 mb-8 pb-8 border-b border-slate-700">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-white text-xl font-bold">{resource.downloads || 0}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
        ...
        {canViewInline() && (
          <div className="glass-lg rounded-2xl p-8 mt-8">
            ...
            <div className={`bg-slate-800 rounded-lg p-4 ${fullscreen ? 'fixed inset-4 z-50' : ''}`}>
              {resource.type === 'image' ? (
                ...
                <div style={{ height: fullscreen ? 'calc(100vh - 8rem)' : '600px' }}>
                  <iframe
                    src={`${API_BASE_URL}/api/v1/resources/${id}/view`}
          ...
          <div className="glass-lg rounded-2xl p-8 mt-8">
            <div className="text-center py-8">
              ...
              <p className="text-slate-400 mb-6">
                This file type ({resource.type.toUpperCase()}) cannot be previewed in the browser.
